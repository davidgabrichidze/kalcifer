defmodule Kalcifer.Engine.JourneyServerTest do
  use Kalcifer.DataCase, async: false

  alias Kalcifer.Engine.JourneyServer
  ...
  defp start_server(graph, opts \\ []) do
    journey = Keyword.get_lazy(opts, :journey, fn -> insert(:journey) end)
    ...
    {:ok, pid} =
      DynamicSupervisor.start_child(
        Kalcifer.Engine.JourneySupervisor,
        {JourneyServer, args}
      )
  ...
  describe "simple graph execution (entry → exit)" do
    test "executes entry and exit nodes, then completes" do
      ...
      assert Enum.at(steps, 1).node_type == "journey_exit"
    ...
    test "instance is marked completed after execution" do
      ...
      instance =
        Repo.one(
          from i in Kalcifer.Journeys.JourneyInstance,
  ...
  describe "branching graph execution" do
    test "executes condition node and follows true branch" do
      ...
      {pid, ref, _args} = start_server(graph, journey: journey)
      assert :ok = wait_for_completion(pid, ref)
      ...
      assert "send_email" in node_types or "journey_exit" in node_types
    end
  end
