defmodule Kalcifer.AI.Client do
  ...
  def default_system_prompt do
    ...
    ## ფლოუს აგების წესრიგი (ყოველთვის დაიცავი!)
    ...
    4. დასტურის შემდეგ: create_flow → add_node სათითაოდ → get_flow_graph → შედეგი უთხარი.
    - ყოველი add_node-ის result-ში current_nodes ჩანს — ამით იცი რა გაქვს.
    - ყველა ნაბიჯის დამატების შემდეგ get_flow_graph-ით გადაამოწმე სტრუქტურა.
    - ᲐᲠᲐᲡᲝᲓᲔᲡ ფიქრო რომ ნაბიჯი დაემატა, სანამ add_node-ის result-ს არ ნახავ.
